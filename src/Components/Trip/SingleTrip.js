...
class Singletrip extends Component {
  ...
  componentDidMount() {
    ...
    Axios.get(
      "https://travel-to-api.herokuapp.com/api/trips/" +
        ...
        if (res.data.status === "success") {
          ...
          Geocode.fromAddress(this.state.trip.from).then(
            response => {
              ...
              if (lat && lng) {
                this.setState(state => {
                  const tripList = state.locationArray.push({
                    city: "A",
                    lat: lat,
                    lng: lng
                  });
    ...
    return (
      <div>
        ...
        <Animated
          ...
          isVisible={this.state.openTripRequestWindow}
          animationIn="fadeIn"
          animationOut="fadeOut"
        >
          <Triprequest
            maxSpace={this.state.trip.number_of_passengers}
            tripID={this.state.trip.id}
            onSuccess={this.manageTripRequest}
          />
        </Animated>
        <Animated
          style={{ position: "fixed", zIndex: "1" }}
          animateOnMount={false}
          isVisible={this.state.openCancelWindow}
          animationIn="fadeIn"
          animationOut="fadeOut"
        >
          <Cancelwindow
            tripID={this.state.trip.id}
            onSuccess={this.manageCancel}
          />
        </Animated>
        <Animated
          style={{ position: "fixed", zIndex: "1" }}
          animateOnMount={false}
          isVisible={this.state.openEdit}
          animationIn="fadeIn"
          animationOut="fadeOut"
        >
        ...
        <div className="single-trip-wrap">
          <div className="map-wrap">
            <div>
              <GoogleMapReact
                ...
                bootstrapURLKeys={{
                  key: "AIzaSyAJeBhbJk8JPhtZsZaY8RO9E230UyPXdy8"
                }}
          ...
          <div className="trip-info-wrap">
            <div className="trip-cities">
              {this.state.trip.from} - {this.state.trip.to}
            </div>
            <span className="trip-dates">
              Leaving on {this.getDate(this.state.trip.departure_time)}
            </span>
            <span className="trip-dates">
              Number of passengers: {this.state.trip.number_of_passengers}
            </span>
            <span className="trip-dates">
              Price per passanger: {this.state.trip.price}EUR
            </span>

            {this.state.trip.driver_id === this.props.user.id ? (
              <div className="flex-row flex-start-center">
                <Button
                  onClick={() => this.manageEdit(true)}
                  variant="primary"
                ...
                <Button
                  onClick={() => this.manageCancel(true)}
                  variant="primary"
                  className="contact-button"
                  type="submit"
                >
            ...
            {this.state.trip.driver_id !== this.props.user.id ? (
              <Button
                onClick={() => this.manageTripRequest(true)}
                variant="primary"
                className="contact-button"
                type="submit"
              >
