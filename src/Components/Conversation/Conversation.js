...
class Conversation extends Component {
  ...
  componentDidMount() {
    ...
    Axios.get(
      "https://travel-to-api.herokuapp.com/api/conversations/" +
  ...
  render() {
      ...
      if (form.formGroupMessage.value) {
        Axios.post(
            ...
            "/message",
          {
            ...
            receiver_id:
              this.props.user.id ===
              this.state.conversation.messages[0].receiver_id
                ? this.state.conversation.messages[0].sender_id
                : this.state.conversation.messages[0].receiver_id
          },
          config
        )
    ...
    const setusuerState = status => {
      ...
      Axios.put(
        "https://travel-to-api.herokuapp.com/api/tripRequests/" +
          this.state.conversation.tripRequest.id +
          "/" +
          status,
        {},
        config
        ...
        .then(res => {
          ...
          if (res.data.status === "success") {
            toast.success("Trip request was successfully " + status + "ed");
          }
        })
        .catch(err => {
          if (status === 'cancel') {
    ...
    return (
      <div>
        <Topbar
          ...
          extraInfo={this.state.conversation}
        ></Topbar>
        <div className="message-wrap">
            ...
            {this.state.conversation.messages
              ? this.state.conversation.messages.map(message => {
                  return (
                    <div
                      className={
                        message.sender_id === this.props.user.id
                          ? "my-message message"
                          : "message"
                      }
                    >
          ...
          this.state.conversation.tripRequest &&
          this.state.conversation.tripRequest.status === "Pending" ? (
            <div className="flex-center-center">
              <Button
                variant="primary"
                type="submit"
                onClick={() => {
                  setusuerState("accept");
                }}
              >
                Accept passenger
              </Button>
          ...
          {this.props.user.role === "passenger" &&
          this.state.conversation.tripRequest &&
          this.state.conversation.tripRequest.status === "Pending" ? (
            <div className="flex-center-center">
              <Button
